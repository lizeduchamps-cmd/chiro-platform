...
async function upsertUser({ discordId, discordUsername, naam, email }) {
  const { data: existing } = await supabaseAdmin
    .from("users")
    .select("id, platform_recht, verantwoordelijkheden")
  ...
  if (existing) {
    await supabaseAdmin
      ...
      .eq("id", existing.id);
    return { id: existing.id, platformRecht: existing.platform_recht, verantwoordelijkheden: existing.verantwoordelijkheden };
  }
  ...
  if (placeholder) {
    ...
    return { id: placeholder.id, platformRecht: placeholder.platform_recht, verantwoordelijkheden: placeholder.verantwoordelijkheden };
  }
  ...
  return { id: created.id, platformRecht: created.platform_recht, verantwoordelijkheden: created.verantwoordelijkheden };
}
...
export const authOptions = {
  ...
  callbacks: {
    async jwt({ token, account, profile }) {
      if (account && profile) {
        const { id, platformRecht, verantwoordelijkheden } = await upsertUser({
        ...
        // Actuele platformrecht/verantwoordelijkheden ophalen, zodat een
        // wijziging door een admin snel doorwerkt zonder opnieuw in te loggen.
        const { data } = await supabaseAdmin
          .from("users")
          .select("platform_recht, verantwoordelijkheden")
          .eq("id", token.userId)
          .maybeSingle();
        if (data) {
