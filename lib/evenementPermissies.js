...
export async function magEvenementBewerken(session, evenementId) {
  ...
  const [{ data: userVerant }, { data: evenement }] = await Promise.all([
    supabaseAdmin.from("user_verantwoordelijkheden").select("verantwoordelijkheden(naam)").eq("user_id", session.user.userId),
    supabaseAdmin.from("evenementen").select("naam").eq("id", evenementId).maybeSingle(),
  ]);
  if (!evenement) return false;
  const verantwoordelijkheden = (userVerant || []).map((r) => r.verantwoordelijkheden?.naam).filter(Boolean);
  return evenementMatchTag(verantwoordelijkheden, evenement.naam);
}
...
export async function evenementIdVanCategorie(categorieId) {
  const { data } = await supabaseAdmin.from("evenement_categorieen").select("evenement_id").eq("id", categorieId).maybeSingle();
  return data?.evenement_id || null;
}
